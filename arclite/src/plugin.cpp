		...
		int Move, const wchar_t **DestPath, int OpMode)
{
	...
	FAR_ERROR_HANDLER_BEGIN
	if (ArcAPI::have_virt_destructor())
		reinterpret_cast<Plugin<true> *>(hPlugin)->get_files(PanelItem, ItemsNumber, Move, DestPath, (OPERATION_MODES)OpMode);
	else
		reinterpret_cast<Plugin<false> *>(hPlugin)->get_files(PanelItem, ItemsNumber, Move, DestPath, (OPERATION_MODES)OpMode);
		...
		int Move, const wchar_t *SrcPath, int OpMode)
{
	FAR_ERROR_HANDLER_BEGIN
	if (ArcAPI::have_virt_destructor())
		reinterpret_cast<Plugin<true> *>(hPlugin)->put_files(PanelItem, ItemsNumber, Move, SrcPath, (OPERATION_MODES)OpMode);
	else
		reinterpret_cast<Plugin<false> *>(hPlugin)->put_files(PanelItem, ItemsNumber, Move, SrcPath, (OPERATION_MODES)OpMode);
...
DeleteFilesW(HANDLE hPlugin, struct PluginPanelItem *PanelItem, int ItemsNumber, int OpMode)
{
	...
	FAR_ERROR_HANDLER_BEGIN
	if (ArcAPI::have_virt_destructor())
		reinterpret_cast<Plugin<true> *>(hPlugin)->delete_files(PanelItem, ItemsNumber, (OPERATION_MODES)OpMode);
	else
		reinterpret_cast<Plugin<false> *>(hPlugin)->delete_files(PanelItem, ItemsNumber, (OPERATION_MODES)OpMode);
...
SHAREDSYMBOL int WINAPI MakeDirectoryW(HANDLE hPlugin, const wchar_t **Name, int OpMode)
{
	...
	FAR_ERROR_HANDLER_BEGIN
	if (ArcAPI::have_virt_destructor())
		reinterpret_cast<Plugin<true> *>(hPlugin)->create_dir(Name, (OPERATION_MODES)OpMode);
	else
		reinterpret_cast<Plugin<false> *>(hPlugin)->create_dir(Name, (OPERATION_MODES)OpMode);
...
ProcessHostFileW(HANDLE hPlugin, struct PluginPanelItem *PanelItem, int ItemsNumber, int OpMode)
{
	...
	if (item == 0) {
		...
		if (ArcAPI::have_virt_destructor())
			reinterpret_cast<Plugin<true> *>(hPlugin)->test_files(PanelItem, ItemsNumber, (OPERATION_MODES)OpMode);
		else
			reinterpret_cast<Plugin<false> *>(hPlugin)->test_files(PanelItem, ItemsNumber, (OPERATION_MODES)OpMode);
