...
void ProgressMonitor::display()
{
	...
	if (progress_known) {
		Far::set_progress_state((PROGRESSTATE)TBPF_NORMAL);
		Far::set_progress_value(percent_done, 100);
	} else {
		Far::set_progress_state((PROGRESSTATE)TBPF_INDETERMINATE);
	}
...
void ProgressMonitor::clean()
{
	if (h_scr) {
		...
		Far::set_progress_state((PROGRESSTATE)TBPF_NOPROGRESS);
		h_scr = nullptr;
	}
