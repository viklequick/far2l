...
/* Mini-parsder of colors to map it directly to RGB */
...
static FarTrueColor index_to_FarTrueColor(int i) {
    if (i < 16) {
        ...
        static const FarTrueColor system_colors[16] = {
            ...
            {0xaa,0xaa,0xaa,1}, {255,0,0,1}, {0,255,0,1}, {255,255,0,1}, {0,0,255,1}, {255,0,255,1}, {0,255,255,1}, {255,255,255,1}
        };
        ...
        // Grayscale ramp
        int gray = (i - 232) * 10 + 8;
        return { (unsigned char)gray, (unsigned char)gray, (unsigned char)gray, 1 };
    }
}
            ...
            for (size_t c = 0; c < codes.size(); ++c) {
                ...
                if (code == 0) {
                    ...
                    if (c + 2 < codes.size() && codes[c+1] == 5) {
                        ...
                        FarTrueColor color = {
                        	(unsigned char)codes[c+2], 
                        	(unsigned char)codes[c+3], 
                        	(unsigned char)codes[c+4] };
                        if (is_fg) current.fg = color; else current.bg = color;
                        c += 4;
                    }
                }
