...
static int uc_newindex(lua_State* L)
{
	...
	{
		fuc->VBuf[index].Char.UnicodeChar = (WCHAR)luaL_checkinteger(L, -1);
	}
	else
	{
		const wchar_t* Char = check_utf8_string(L, -1, NULL);
		fuc->VBuf[index].Char.UnicodeChar = Char[0];
	}
	lua_pop(L, 1);
...
{
	TFarUserControl* fuc = (TFarUserControl*)lua_newuserdata(L, sizeof(TFarUserControl) + sizeof(CHAR_INFO)*(X*Y-1));
	fuc->X = X;
	fuc->Y = Y;
	fuc->Size = X*Y;
